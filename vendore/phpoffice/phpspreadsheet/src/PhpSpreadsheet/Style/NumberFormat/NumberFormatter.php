...
class NumberFormatter extends BaseFormatter
{
    ...
    public static function format(mixed $value, string $format): string
    {
        ...
        // on Excel shows this format uses Euro symbol, not ($_SESSION['user']['preferred_currency'] ?? ($_SESSION['entreprise']['devise'] ?? 'FCFA')).
        // if ($format === NumberFormat::FORMAT_CURRENCY_EUR_SIMPLE) {
        //     return ($_SESSION['user']['preferred_currency'] ?? ($_SESSION['entreprise']['devise'] ?? 'FCFA')) . sprintf('%1.2f', $value);
        // }
